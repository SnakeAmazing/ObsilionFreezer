...
public class FreezeCommand implements CommandExecutor {
    ...
    public FreezeCommand(ObsilionFreezer plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (target.hasPermission("obsilionfreezer.exempt")) {
            player.sendMessage(MessageDecorator.decorate(config.cantFreezeStaff()));
            return true;
        }

        if (frozenHandler.contains(target)) {
            player.sendMessage(MessageDecorator.decorate(config.alreadyFrozen()));
            return true;
        }

        frozenHandler.add(target, player);
